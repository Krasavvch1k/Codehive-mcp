"""
Worqen-specific tools для MCP-сервера CodeHive.

Експортує:
- WORQEN_TOOLS — список з 33 Tool() оголошень (з префіксом worqen_*)
- worqen_dispatch(name, args, format_response) — синхронний диспатчер.
  Повертає list[TextContent] якщо tool обробився, або None — щоб обгортка
  спробувала наступного диспатчера (наприклад codehive_dispatch).

Обгортка call_tool у server.py відповідає за async-декоратор, exception
handling і формат відповіді (через _format_response).
"""
...
from projects.worqen.parsers import docs as docs_parser
from projects.worqen.parsers import analytics
from projects.worqen.parsers import team_discussions as team_parser
from projects.worqen.parsers.references import validate_references
from projects.worqen.writers.us_writer import update_story, create_story
...
WORQEN_TOOLS: list[Tool] = [
    # ---------- User Stories ----------
    Tool(
        name="worqen_list_user_stories",
        description=(
    ...
    Tool(
        name="worqen_next_us_id",
        description="Наступний вільний US-XXX.",
        inputSchema={"type": "object", "properties": {}},
    ),
    Tool(
        name="worqen_epics_summary",
        description=(
    ...
    Tool(
        name="worqen_find_us_dependents",
    ...
    Tool(
        name="worqen_next_bug_id",
        description="Наступний вільний BUG-XXX.",
        inputSchema={"type": "object", "properties": {}},
    ),
    Tool(
        name="worqen_bugs_summary",
    ...
    Tool(
        name="worqen_validate_id_format",
    ...
    Tool(
        name="worqen_list_open_questions",
        description=(
    ...
    Tool(
        name="worqen_read_team_discussion",
        description=(
    ...
    Tool(
        name="worqen_search_team_discussions",
        description=(
    ...
    # ---------- Utils ----------
    Tool(
        name="worqen_get_session_writes",
        description=(
        ...
        inputSchema={
            ...
            "properties": {
                ...
                "tool": {
                    "type": "string",
                    "enum": [
                        "worqen_update_user_story",
                        "worqen_create_user_story",
                        "worqen_update_bug",
                        "worqen_create_bug",
                    ],
    ...
    Tool(
        name="worqen_bulk_validate_references",
        description=(
...
def worqen_dispatch(name: str, args: dict, format_response):
    """
    Синхронний диспатчер для 33 worqen tools.

    Повертає list[TextContent] якщо tool обробився, або None — щоб обгортка
    спробувала наступного диспатчера (codehive_dispatch тощо).

    format_response — ін'єкується з server.py, щоб уникнути circular import.
    """
    # ---------- User Stories ----------
    if name == "worqen_list_user_stories":
        ...
        return format_response(format_list_response(
            result,
    ...
    if name == "worqen_get_user_story":
        result = us_parser.get_story(args["id"])
        if result is None:
            return format_response({"error": f"US '{args['id']}' не знайдено"})
        return format_response(strip_empty(result))

    if name == "worqen_next_us_id":
        return format_response({"next_id": us_parser.get_next_us_id()})

    if name == "worqen_epics_summary":
        return format_response(us_parser.list_epics_summary(level=args.get("level", "minimal")))

    if name == "worqen_find_us_dependents":
        tier = args.get("tier", "scan")
        result = us_parser.find_dependents(args["id"], tier=tier)
        cols = US_SCAN_COLUMNS if tier == "scan" else None
        return format_response(format_list_response(result, table_columns=cols))

    # ---------- Bug Report ----------
    if name == "worqen_list_bugs":
        tier = args.get("tier", "scan")
    ...
    if name == "worqen_get_bug":
        result = bug_parser.get_bug(args["id"])
        if result is None:
            return format_response({"error": f"BUG '{args['id']}' не знайдено"})
        return format_response(strip_empty(result))

    if name == "worqen_next_bug_id":
        return format_response({"next_id": bug_parser.get_next_bug_id()})

    if name == "worqen_bugs_summary":
        return format_response(bug_parser.bugs_summary(level=args.get("level", "full")))

    if name == "worqen_list_ficha":
        tier = args.get("tier", "scan")
    ...
    # ---------- Documents ----------
    if name == "worqen_list_doc_sections":
        result = docs_parser.get_toc(args["doc"])
        return format_response({"count": len(result), "sections": result})

    if name == "worqen_get_doc_section":
    ...
    if name == "worqen_get_doc_full_text":
    ...
    # ---------- Analytics ----------
    if name == "worqen_detect_conflicts":
        return format_response(analytics.detect_conflicts(
            epic=args.get("epic"),
            severity_min=args.get("severity_min", "warning"),
            limit=args.get("limit", 20),
        ))

    if name == "worqen_validate_id_format":
        return format_response(analytics.validate_id_format(
            entity=args.get("entity", "us"),
        ))

    if name == "worqen_list_open_questions":
        return format_response(analytics.list_open_questions(
            epic=args.get("epic"),
        ))

    if name == "worqen_session_changes":
        return format_response(analytics.session_changes(
            since_date=args.get("since_date"),
        ))

    if name == "worqen_save_snapshot":
        path = analytics.save_today_snapshot()
        return format_response({"saved_to": path})

    if name == "worqen_list_snapshots":
        return format_response({"snapshots": analytics.list_snapshots()})

    # ---------- Team Discussions ----------
    if name == "worqen_list_team_discussions":
        return format_response(team_parser.list_team_discussions(
            status=args.get("status"),
            session_date=args.get("session_date"),
            limit=args.get("limit", 50),
        ))

    if name == "worqen_read_team_discussion":
        try:
            return format_response(team_parser.read_team_discussion(args["query"]))
        except ValueError as ve:
            return format_response({"error": str(ve)})

    if name == "worqen_search_team_discussions":
        try:
            return format_response(team_parser.search_team_discussions(
                query=args["query"],
    ...
    # ---------- Writers ----------
    if name == "worqen_update_user_story":
        try:
            return format_response(update_story(
                args["id"],
                args.get("fields") or {},
    ...
    if name == "worqen_create_user_story":
        try:
            return format_response(create_story(
                args.get("fields") or {},
    ...
    if name == "worqen_update_bug":
        try:
            return format_response(update_bug(
                args["id"],
                args.get("fields") or {},
                force_overwrite=bool(args.get("force_overwrite", False)),
            ))
    ...
    if name == "worqen_create_bug":
        try:
            return format_response(create_bug(
                args.get("fields") or {},
                use_id=args.get("use_id"),
                force_overwrite=bool(args.get("force_overwrite", False)),
            ))
    ...
    # ---------- Utils ----------
    if name == "worqen_get_session_writes":
        date = args.get("date")
        if date:
    ...
    if name == "worqen_bulk_validate_references":
        return format_response(validate_references(
    ...
    if name == "worqen_list_files":
        ...
        return format_response(files_info)

    if name == "worqen_clear_cache":
        clear_drive_cache()
        return format_response({"status": "cache cleared"})

    if name == "worqen_check_drive_sync":
        file_key = args["file_key"]
        if file_key not in FILE_IDS:
            return format_response({"error": f"Unknown file_key '{file_key}'"})
        return format_response(is_drive_newer(FILE_IDS[file_key]))
